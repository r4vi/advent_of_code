...
def apply_move(game_state, n, from_, to):
    ...
    for i in range(n):
        while not to_move:
            to_move = game_state[from_zero_idx].pop()
        game_state[to_zero_idx].append(to_move)
        to_move = None
...
def main():
    ...
    # sample = Path("./sample").open("r").read()
    #
    # state, moves = parse_input(sample)
    # game_state = build_game(state)
    # pprint(game_state)
