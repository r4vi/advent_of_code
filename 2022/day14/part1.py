...
class Grid:
    ...
    def is_blocked(self, loc):
        return (loc in self.rocks) or (not self.is_in_bounds(loc))
    ...
    def next_coord(self, sand):
        if not self.is_blocked(new := apply_move(sand, DOWN)): return new
        elif not self.is_blocked(new := apply_move(sand, LEFT_DOWN)): return new
        elif not self.is_blocked(new := apply_move(sand, RIGHT_DOWN)): return new
        self.sand.add(sand)
        self.rocks.add(sand)
        return None
    ...
    def run(self):
        ...
        while not self.source in self.sand:
            ...
            if curr and not self.is_in_bounds(curr):
                break
            if curr is None:
                self.sand.add(prev)
                self.rocks.add(prev)
                # start from the top
                prev = curr = self.source
            else:
                prev = curr
